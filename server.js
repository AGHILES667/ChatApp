...
app.post('/signup', (req, res) => {
  connection.query(
    ...
    (error, results) => {
      req.flash('login', 'Féclicitations, votre compte est désormais actif...');
      res.redirect('/');
    }
  )
});
    ...
    (error, results) => {
      if (results.length > 0) {
        ...
        res.redirect('/list');
				...
				req.flash('login', 'Nom d\'utilisateur ou mot de passe incorrecte !');
        res.redirect('/');
      }
    }
  );
...
app.post('/uploadpp', function(req, res) {
  ...
  if (req.session.loggedin) {
    if (!req.files || Object.keys(req.files).length === 0) {
      return res.status(400).send('Aucun fichier chargé');
    }else if (thefile.mimetype.split('/')[0] != 'image') {
      return res.status(400).send('Le fichier doit etre une image.');
    }
    let name_file = +new Date() + thefile.md5;
    let path = __dirname + '/img/' + name_file + '.' + thefile.name.split('.')[1];
    let for_save = '/img/' + name_file + '.' + thefile.name.split('.')[1];
...
io.sockets.on('connection', function(socket) {
  ...
  console.log('Plusieurs appareils connectés : %s', yangterhubung.length);
  //disconnect
  socket.on('disconnect', function(data) {
    yangterhubung.splice(yangterhubung.indexOf(socket), 1)
    console.log('Déconnceté: %s ', yangterhubung.length)
  })
